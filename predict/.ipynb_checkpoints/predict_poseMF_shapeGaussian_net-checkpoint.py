...
from utils.sampling_utils import compute_vertex_uncertainties_by_poseMF_shapeGaussian_sampling, \
    joints2D_error_sorted_verts_sampling
from utils.gaussian_product_fusion import gaussian_product_fusion_tensor
...
def predict_poseMF_shapeGaussian_net(pose_shape_model,
    ...
    my add new function: gaussian product to fusion the β params
    """
    ...
    output_means_covariances_dic = {}

    def prdict_single(image_fname):
        with torch.no_grad():
            ...
            proxy_rep_img = edge_detector_output['thresholded_thin_edges'] if pose_shape_cfg.DATA.EDGE_NMS else \
            ...
            hrnet_joints2Dvisib[
                [0, 1, 2, 3, 4, 5, 6, 11, 12]] = True  # Only removing joints [7, 8, 9, 10, 13, 14, 15, 16] if occluded
            ...
            output_means_covariances_dic[image_fname] = {'mean':pred_shape_dist.loc, 'cov':pred_shape_dist.scale}

            # output mesh file
            pred_vertices = pred_vertices_mode[0].cpu().numpy()
            mesh_save_path = os.path.join(save_dir, image_fname)
            outmesh = os.path.splitext(mesh_save_path)[0] + '_smpl.obj'
            with open(paths.SMPL_MODEL_DIR, 'rb') as fp:
                u = pkl._Unpickler(fp)
                u.encoding = 'latin1'
                model = u.load()
            pred_faces = model['f']
            ...
            pred_reposed_vertices_flipped_mean = aa_rotate_translate_points_pytorch3d(points=pred_reposed_vertices_mean,
                                                                                      ...
                                                                                      translations=torch.zeros(3,
                                                                                                               device=device))
            # Rotating 90° about vertical axis for visualisation
            pred_reposed_vertices_rot90_mean = aa_rotate_translate_points_pytorch3d(
                points=pred_reposed_vertices_flipped_mean,
            ...
            if visualise_samples:
                ...
                pred_vertices_samples = joints2D_error_sorted_verts_sampling(
                    pred_vertices_samples=pred_vertices_samples,
                    pred_joints_samples=pred_joints_samples,
                    input_joints2D_heatmaps=proxy_rep_input[:, 1:,
                                            :, :],
                    pred_cam_wp=pred_cam_wp)[:num_samples, :,
                                        :]  # (8, 6890, 3)
                # Need to flip pred_vertices before projecting so that they project the right way up.
                ...
                pred_vertices_rot90_samples = torch.cat([pred_vertices_rot90_mode, pred_vertices_rot90_samples],
                                                        dim=0)  # (9, 6890, 3)
            ...
            # Cropped input image
            combined_vis_fig[:visualise_wh, :visualise_wh] = cropped_for_proxy_rgb.cpu().detach().numpy()[0].transpose(
                1, 2,
                0)
            ...
            for joint_num in range(cropped_for_proxy['joints2D'].shape[1]):
                hor_coord = cropped_for_proxy['joints2D'][
                                0, joint_num, 0].item() * visualise_wh / pose_shape_cfg.DATA.PROXY_REP_SIZE
                ver_coord = cropped_for_proxy['joints2D'][
                                0, joint_num, 1].item() * visualise_wh / pose_shape_cfg.DATA.PROXY_REP_SIZE
            ...
            3 * visualise_wh:4 * visualise_wh] = reposed_body_vis_rgb_rot90
            ...
            if visualise_samples:
                ...
                for i in range(num_samples + 1):
                    ...
                    body_vis_rgb_sample = batch_add_rgb_background(backgrounds=cropped_for_proxy_rgb,
                                                                   rgb=body_vis_output_sample['rgb_images'].permute(0,
                                                                                                                    3,
                                                                                                                    1,
                                                                                                                    2).contiguous(),
                                                                   seg=body_vis_output_sample['iuv_images'][:, :, :,
                                                                       0].round())
                    ...
                    samples_fig[row * visualise_wh:(row + 1) * visualise_wh,
                    col * visualise_wh:(col + 1) * visualise_wh] = body_vis_rgb_sample

                    row = (2 * i + 1) // samples_cols
                    col = (2 * i + 1) % samples_cols
                    samples_fig[row * visualise_wh:(row + 1) * visualise_wh,
                    col * visualise_wh:(col + 1) * visualise_wh] = body_vis_rgb_rot90_sample

                    samples_fig_save_path = os.path.splitext(vis_save_path)[0] + '_samples.png'
                    cv2.imwrite(samples_fig_save_path, samples_fig[:, :, ::-1] * 255)

            return output_means_covariances_dic

    def prdict_gaussaian_product_fusion(test_dic, image_fname, output_means_covariances_dic):
        with torch.no_grad():
            # ------------------------- INPUT LOADING AND PROXY REPRESENTATION GENERATION -------------------------
            image = cv2.cvtColor(cv2.imread(os.path.join(test_dic, image_fname)), cv2.COLOR_BGR2RGB)
            orig_image = image.copy()
            image = torch.from_numpy(image.transpose(2, 0, 1)).float().to(device) / 255.0
            # Predict Person Bounding Box + 2D Joints
            hrnet_output = predict_hrnet(hrnet_model=hrnet_model,
            ...
            # gaussian product fusion shape param

            output_means_covariances_list = list(output_means_covariances_dic.values())
            shape_mean_list = []
            shape_scale_list = []
            for i in range(len(output_means_covariances_list)):
                shape_mean_list.append(output_means_covariances_list[i]['mean'])
                shape_scale_list.append(output_means_covariances_list[i]['cov'])
            shape_mean_matrix = torch.cat(shape_mean_list)
            shape_scale_matrix = torch.cat(shape_scale_list)


            shape_mean_fusion = gaussian_product_fusion_tensor(shape_mean_matrix, shape_scale_matrix)
            # import ipdb
            # ipdb.set_trace()

            pred_smpl_output_mode = smpl_model(body_pose=pred_pose_rotmats_mode,
                                               global_orient=pred_glob_rotmats.unsqueeze(1),
                                               betas=shape_mean_fusion.unsqueeze(0),
                                               pose2rot=False)
            # pred_smpl_output_mode = smpl_model(body_pose=mean_pose.unsqueeze(0),
            #                                    global_orient=mean_glob_romats.unsqueeze(0).unsqueeze(1),
            #                                    betas=mean_shape.unsqueeze(0),
            #                                    pose2rot=False)
            ...
            pred_vertices = pred_vertices_mode[0].cpu().numpy()
            mesh_save_path = os.path.join(save_dir, image_fname)
            outmesh = os.path.splitext(mesh_save_path)[0] + '_fusion_smpl.obj'
            with open(paths.SMPL_MODEL_DIR, 'rb') as fp:
                u = pkl._Unpickler(fp)
                u.encoding = 'latin1'
                model = u.load()
            ...
            combined_vis_fig[visualise_wh:2 * visualise_wh,
            3 * visualise_wh:4 * visualise_wh] = reposed_body_vis_rgb_rot90
            # fusion_image_fname = 'fusion_' + image_fname
            vis_save_path = os.path.join(save_dir, image_fname)
            cv2.imwrite(vis_save_path, combined_vis_fig[:, :, ::-1] * 255)
            ...
            if visualise_uncropped:
                ...
                uncropped_vis_save_path = os.path.splitext(vis_save_path)[0] + '_fusion_uncrop.png'
                cv2.imwrite(uncropped_vis_save_path, uncropped_rgb_with_background[:, :, ::-1])
            ...
            if visualise_samples:
                ...
                for i in range(num_samples + 1):
                    ...
                    samples_fig_save_path = os.path.splitext(vis_save_path)[0] + '_fusion_samples.png'
                    cv2.imwrite(samples_fig_save_path, samples_fig[:, :, ::-1] * 255)

    for image_fname in tqdm(sorted([f for f in os.listdir(image_dir) if f.endswith(('.jpg', '.png'))])):
        output_means_covariances_dic = prdict_single(image_fname)
    front_image = '1643143575739080705.jpg'
    front_dic = 'testimgs2'
    prdict_gaussaian_product_fusion(front_dic, front_image, output_means_covariances_dic)
